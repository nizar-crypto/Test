...
const clubArray = clubs.map((club) => {
  console.log("club memeber", club);
  const nationalities = club.members.map((id) => {
    console.log("id", id);
    const findId = people.find((person) => {
      console.log("check id", person.id, "id club", id), id === person.id;
    });
    return findId;
  });
