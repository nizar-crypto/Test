...
// 1. Desired output:
// Array of projects which are active
...
for (let index = 0; index < projects.length; index++) {
  const project = projects[index];
  // console.log("check active project", activeProject);

  var activeProject = project.active;
  console.log("check activePro", activeProject, activeProject === true);
  if (activeProject === true) {
    // console.log("CHECK IF", project);
    output.push(project);
    // } else if ((activeProject = false)) {
    //   console.log("checccck not active");
  }
}
console.log(output);
