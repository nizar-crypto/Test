...
const cheapestProduct = products.reduce((accu, product) => {
  console.log("check accumlator", accu, "CHECK NUMBER", product.cost);
  if (accu === null) {
    return product.cost;
  } else if (accu > product.cost) {
    return product.cost;
  } else if (accu < product.cost) {
    return accu;
  }
}, null);
console.log("check", cheapestProduct);
